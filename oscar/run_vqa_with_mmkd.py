...
class VQADataset(Dataset):
    ...
    def __init__(self, args, name, tokenizer):
        ...
        # load image features
        self.img_features = _load_img_features(args, name)
    ...
    def tensorize_example(self,
        ...
        input_mask_txt = [1 if mask_padding_with_zero else 0] * len(input_ids)
        input_mask_img = [0 if mask_padding_with_zero else 0] * len(input_ids)
...
def _load_dataset(args, name):
    ...
    if name == 'train':
        if args.data_label_type == 'mask':
            ...
            else:  # train2014_qla_mrcnn.json
                examples = processor.get_train_examples(args.data_dir, 'train2014_qla_mrcnn.json')
        else:
            examples = processor.get_train_examples(args.data_dir, 'train2014_qla.json')
    elif name == 'train+val':
        if args.data_label_type == 'mask':  # train+val2014_qla_mrcnn.json
        ...
        if args.data_label_type == 'mask':
            ...
            else:  # val2014_qla_mrcnn.json
                examples = processor.get_dev_examples(args.data_dir, 'val2014_qla_mrcnn.json')
        else:
            examples = processor.get_dev_examples(args.data_dir, 'val2014_qla.json')
    elif name == 'test2015':
        if args.data_label_type == 'mask':  # test2015_qla_mrcnn.json
        ...
        if args.data_label_type == 'mask':  # test-dev2015_qla_mrcnn.json
...
def _load_img_features(args, name):
    ...
    if args.img_feature_type == 'faster_r-cnn':  # faster_r-cnn
        if args.img_feature_dim == 2048:  # object features: 2048
            ...
            feat_file_name = '{}_img_frcnn_feats.pt'.format(name)  # train_img_frcnn_feats.pt
    else:
...
def compute_score_with_logits(logits, labels):
    logits = torch.max(logits, dim=1)[1].data
    one_hots = torch.zeros(*labels.size()).cuda()
    one_hots.scatter_(dim=1, index=logits.view(-1, 1), value=1)
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {
                ...
                #
                'attention_mask_txt': batch[7],
                'attention_mask_img': batch[8]
            }
        ...
        if (args.local_rank in [-1, 0]) and (args.save_epoch > 0 and epoch % args.save_epoch == 0) and (epoch > args.save_after_epoch):
            ...
            # teacher_output_dir = os.path.join(base_output_dir, 'teacher')  # ./model/vqa/student/checkpoint-1/teacher
            # student_output_dir = os.path.join(base_output_dir, 'student')  # ./model/vqa/student/checkpoint-1/student
            ...
            logger.info("Saving checkpoint {0} to {1}".format(epoch + 1, output_dir))
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                ...
                batch_score = torch.sum(compute_score_with_logits(logits, batch[4]), dim=1)
...
def test(args, model, eval_dataset=None, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        with open(args.output_dir + ('/{}_results.json'.format(eval_dataset.name)), 'w') as fp:  # model/vqa/student/test2015_results.json
