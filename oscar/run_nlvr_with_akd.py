        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if phase == 1:
                ...
                if args.loss_type == 'bce':
                    ...
                    loss = loss_fct(student_logits, inputs['labels'].view(-1, args.num_labels))
                    loss *= inputs['labels'].size(1)
                elif args.loss_type == 'ce':
                    loss_fct = nn.CrossEntropyLoss(reduction='mean')
                    loss = loss_fct(student_logits, inputs['labels'].view(-1))
...
def test(args, model, eval_dataset=None, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        with open(args.output_dir + ('/{}_results.json'.format(eval_dataset.name)), 'w') as fp:  # model/nlvr2/best/xxx_results.json
