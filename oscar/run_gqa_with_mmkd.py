...
def _load_dataset(args, name):
    ...
    if name == 'train':
        ...
        if args.train_data_type == 'bal':
            ...
            examples = processor.get_train_examples(args.data_dir, 'gqa_all_qla_train.json')
    elif name == 'val':
        # if args.data_label_type == 'mask':
        if args.eval_data_type == 'bal':
            examples = processor.get_dev_examples(args.data_dir, 'gqa_bal_qla_val.json')
        else:  # all
            examples = processor.get_dev_examples(args.data_dir, 'gqa_all_qla_val.json')
    elif name == 'test':
        # if args.data_label_type == 'mask':
...
def train(args, train_dataset, eval_dataset, student_model, teacher_model, tokenizer):
    ...
    for epoch in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            # L_task_specific
            task_specific_loss, fast_logits = fast_outputs[:2]
        ...
        if (args.local_rank in [-1, 0]) and (args.save_epoch > 0 and epoch % args.save_epoch == 0) and (epoch > args.save_after_epoch):
            # base_output_dir = os.path.join(args.output_dir, 'checkpoint-{}'.format(epoch + 1))  # ./model/gqa/student/checkpoint-1
            # teacher_output_dir = os.path.join(base_output_dir, 'teacher')  # ./model/gqa/student/checkpoint-1/teacher
            # student_output_dir = os.path.join(base_output_dir, 'student')  # ./model/gqa/student/checkpoint-1/student
    ...
    if args.local_rank in [-1, 0]:
        output_dir = args.output_dir  # model/gqa/student
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
...
def main():
    ...
    tokenizer = tokenizer_class.from_pretrained(  # BertTokenizer
        args.teacher_model,  # model/gqa/teacher
    ...
    if args.do_train:
        train_dataset = GQADataset(args, 'train', tokenizer, img_features)
        eval_dataset = GQADataset(args, 'val', tokenizer, img_features)
    ...
    if args.do_train_val:
        train_dataset = GQADataset(args, 'train+val', tokenizer, img_features)
        eval_dataset = GQADataset(args, 'val', tokenizer, img_features)
        global_step, tr_loss = train(args, train_dataset, eval_dataset, student_model, teacher_model, tokenizer)
        logger.info(" global_step = %s, average loss = %s", global_step, tr_loss)
    ...
    #     test_dev_dataset = GQADataset(args, 'test-dev', tokenizer, img_features)
    #
    #     checkpoints = [args.output_dir]
    #     if args.eval_all_checkpoints:
    #         checkpoints = list(os.path.dirname(c) for c in sorted(glob.glob(args.output_dir + '/**/' + WEIGHTS_NAME, recursive=True)))
    ...
    #     test_dataset = GQADataset(args, 'test', tokenizer, img_features)
    #
    #     checkpoints = [args.output_dir]
    #     if args.eval_all_checkpoints:
    #         checkpoints = list(os.path.dirname(c) for c in sorted(glob.glob(args.output_dir + '/**/' + WEIGHTS_NAME, recursive=True)))
