...
def train(args, train_dataset, eval_dataset, student_model, teacher_model, tokenizer, phase):
    """ Train the model """
    ...
    if phase == 1:
        train_iterator = trange(int(args.num_train_epochs_phase_1), desc="Epoch", disable=args.local_rank not in [-1, 0])
        temp = 1
    else:
        train_iterator = trange(int(args.num_train_epochs_phase_2), desc="Epoch", disable=args.local_rank not in [-1, 0])
    ...
    for epoch in train_iterator:
        ...
        if (args.local_rank in [-1, 0]) and (args.save_epoch > 0 and epoch % args.save_epoch == 0) and (epoch > args.save_after_epoch):
            output_dir = os.path.join(args.output_dir, 'checkpoint-{}'.format(epoch + 1))  # ./model/vqa/student/checkpoint-1
